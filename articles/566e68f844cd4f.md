...
topics: ["Haskell", "遅延評価", "関数型プログラミング", "fold", "正格評価"]
published: true
---

## これは何?

(この記事は[クロス投稿](https://qiita.com/sigma_devsecops/items/206874ce5130abe280da)です)
...
```haskell
all (>2) xs = foldr (\x e -> (>2) x && e) True [3,2,1]
```

まず、`foldr`の定義通りに展開していく。
...
`&&`は遅延評価により、左側が`False`の場合、右側を評価しない。
